...
import type { BPMNote, Note, NoteIndex, StopNote, TimeSignature, TrackIndex } from '@/utils/scoreTypes';
import { EventNoteProps, MusicNoteProps, PlayNote, PlayNoteProps } from '@/utils/scoreTypes';

import _ from 'lodash';

export function binarySearchIndex<T>(list: Array<T>, matcher: T => boolean): number {
  let
    start = 0,
    end = list.length,
...
export function binarySearch<T>(list: Array<T>, matcher: T => boolean): ?T {
...
export function mergeSortedList<T>(
  la: Array<T>, lb: Array<T>, comparator: (T, T) => boolean,
): Array<T> {
  const list: Array<T> = [];
...
export class NoteManager {
  ...
  getLastNote(): ?Note {
    ...
    let lastNote: ?Note = lastPlayNote;

    [lastBPMNote, lastStopNote].forEach((note: ?Note) => {
      if (!lastNote) {
        lastNote = note;
  ...
  deleteNote(noteIndex: NoteIndex): void {
    const note: ?Note = this.allNotes[noteIndex];
